...
async function findOneById(_id) {
    return await userModel.findById(_id).populate('tasks').exec();
}

async function findOne(email) {
    return await userModel.findOne({ email: email }).populate('tasks').exec();
}
...
async function update(id, updatedUser) {
    return await userModel.findByIdAndUpdate(id, updatedUser, { new: true }).populate('tasks').exec();
}
...
async function addTaskToUser(userId, taskId) {
  try {
      ...
      throw new Error(`Error al agregar tarea al usuario: ${error.message}`);
  }
}
